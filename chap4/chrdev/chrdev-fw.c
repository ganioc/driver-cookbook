...
static int chrdev_load_fw_nowait(struct device *dev, const char *file){
    ...
    ret = request_firmware_nowait(THIS_MODULE, false, fw_name,
        dev, GFP_KERNEL, dev, chrdev_fw_cb);

    if(ret){
        dev_err(dev, "unable to register call back for firmware loading\n");
        return ret;
    }
...
MODULE_DEVICE_TABLE(of, of_chrdev_reg_match);
...
static struct platform_driver chrdev_req_driver = {
    ...
    .driver = {
        .name = "chrdev-fw",
        .of_match_table = of_chrdev_reg_match,
    },
};
